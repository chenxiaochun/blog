...
`gonna`：相当于`going to`的省略版  
`wanna`：相当于`want to`的省略版  
...
`anything`：表示任何一样东西。用于否定和疑问句  
`everything`：表示所有东西。用于肯定句  
...
`disgust`：厌恶、作呕  
`no wonder`：怪不得、难怪  
`No wonder he was disgusted with my`：怪不得他对我如此反感  
...
1. 连字符位于名词之前，用作定语的复合语之间，一般就加连字符  
`Jim is a 15-year-old boy`：Jim是一个十五岁的男孩  

2. 连字符加在两个词中间，使这两个词变成一个词，以避免误解  
